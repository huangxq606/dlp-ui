$.fn.fishBone = function(data) {
    ...
    function creataFishBone(data) {
        ...
        $(data).each(function(index) {
            ...
            if(itemclass=='top'){
                $.each(this, function(name, value) {
                    if (name == 'flowna') {
                        var li = $("<li class='line-first'>" + "操作项：" + value + "</li>")
                ...
                $.each(this, function(name, value) {
                    if (name == 'status') {
                        ...
                        */
                        var li = $("<li class='title'></li>");
                        if(value=='5'){
                        	//var titleLeft =  $("<span class='title-left-red'>&nbsp;</span>").css('background-position-y',titleLeftY);
                        	//var titleCenter =  $("<span class='title-center-red'>"+"失败"+"</span>").css('background-position-y',titleCenterY);
                        	//var titleRight =  $("<span class='title-right-red'>&nbsp;</span>").css('background-position-y',titleRightY);
                        	var statusTitle = $("<span class='statusTitle-red'>"+"   失败"+"</span>").css('background-position-y',titleLeftY);
                        }else if(value=='2'){
                        	//var titleLeft =  $("<span class='title-left-green'>&nbsp;</span>").css('background-position-y',titleLeftY);
                        	//var titleCenter =  $("<span class='title-center-green'>"+"成功"+"</span>").css('background-position-y',titleCenterY);
                        	//var titleRight =  $("<span class='title-right-green'>&nbsp;</span>").css('background-position-y',titleRightY);
                        	var statusTitle = $("<span class='statusTitle-green'>"+"   成功"+"</span>").css('background-position-y',titleLeftY);
                        	
                        }else if(value=='1'){
                        	//var titleLeft =  $("<span class='title-right-purple'>&nbsp;</span>").css('background-position-y',titleLeftY);
                        	//var titleCenter =  $("<span class='title-right-purple'>"+"执行中"+"</span>").css('background-position-y',titleCenterY);
                        	//var titleRight =  $("<span class='title-right-purple'>&nbsp;</span>").css('background-position-y',titleRightY);
                        	var statusTitle = $("<span class='statusTitle-purple'>"+"   执行中"+"</span>").css('background-position-y',titleLeftY);
                        	
                        }
                        //li.append(titleLeft).append(titleCenter).append(titleRight);
                        li.append(statusTitle);
                        li.appendTo(ul);
                        return;
            	 ...
            	 $.each(this, function(name, value) {
	                if (name == 'dplyid') {
	                	...
	                	var li = $("<li>查看日志：<a><img src='assets/plugins/fishBone/img/edit.png' width='24px' height='24px'></a></li>").css("border-left","1px solid "+color);
	                	$(li.find("a")[0]).attr("href","javascript:lookLog('"+value+"')");
	                    
	                    //var li = $("<li>查看日志：<a href='"+value+"'><img src='assets/plugins/fishBone/img/edit.png' width='24px' height='24px'></a></li>").css("border-left","1px solid "+color);
            			li.appendTo(ul);
	                }
            	});
            ...
            //封装其他属性
            $.each(this, function(name, value) {
                if (name != 'status' && name != 'flowna' && name != 'dplyid') {
                    var li = $("<li>" + (name=="extime"?"开始时间":"结束时间") + "：" + value + "</li>").css("border-left","1px solid "+color);
                    li.appendTo(ul);
                }
            });
        ...
        var line = $("<div class='line_fishBone'/>")
